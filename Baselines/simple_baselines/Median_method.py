#baseline
#For each disease, take the temporal or spatial median
...
def Spatial_median(select_diease_id,missing_rate):
    ...
    for yy in range(2008,2009):
        ...
        median = np.median(ave_all)
        #average = sum(ave_all)/len(ave_all)
        #print(average)
        A_result = [median for i in range(483)]
        RMSE, MAE = test_loss(A_result, ward_nor_list, yy, select_diease_id[0], 483)



def Time_median(select_diease_id,missing_rate):
...
if __name__=='__main__':
    ...
    Spatial_median(select_diease_id,missing_rate)
    print()
    Time_median(select_diease_id,missing_rate)